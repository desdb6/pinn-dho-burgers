...
def loss_physics_inverse(cfg: Config, model: nn.Module, t_col_t: torch.Tensor, x_col_t: torch.Tensor) -> torch.Tensor:
    """
    L_physics = mean( r(t_i)^2 )  over collocation points
...
def loss_bc(cfg: Config, model: nn.Module, t_bc_t: torch.Tensor) -> torch.Tensor:
    """
    L_bc = mean(u_hat(x=0)-left_bc)^2 + mean(u_hat(x=L)-right_bc)^2 
    """
