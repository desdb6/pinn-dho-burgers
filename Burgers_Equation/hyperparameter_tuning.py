...
OUTPUT_PATH = Path.cwd() / "Burgers_Equation\outputs\optuna"
...
CSV_FIELDS = [
    "trial", "value",
    "n_obs", "n_col_dom", "hidden", "n_layers", "lr", "scheduler_gamma", "scheduler_step",
...
def objective(trial: optuna.Trial) -> float:
    ...
    cfg = Config(
        ic              = "Gauss",
        n_obs           = trial.suggest_int("n_obs",      20,  300, step=20),
        n_col_dom       = trial.suggest_int("n_col_dom", 100,  500, step=20),
    ...
    row = {
        "trial":            trial.number,
        "value":            best_val,
        "n_obs":            cfg.n_obs,        # add this
        "n_col_dom":        cfg.n_col_dom,    # add this
        "hidden":           cfg.hidden,
