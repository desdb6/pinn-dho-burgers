...
from utils import pointwise_residual, rmse, convert_to_mck
...
def plot_analytic(cfg: Config, t_end: float) -> None:
    """
    ...
    plt.plot(t, y, linewidth=2, label="Analytic solution")
    plt.xlabel("Time $t$", fontsize=12)
    plt.ylabel("Solution $y(t)$", fontsize=12)
    plt.title("Analytic Solution of the Differential Equation", fontsize=14)
...
def plot_summary(
    ...
    device: torch.device,
    save_path: Path = Path("outputs/summary.png"),
    show_plot: bool = True
) -> None:
    """
    ...
    ax_data.scatter(data["t_obs"], data["y_obs"],
                    ...
                    label=f"Train observations  (N={cfg.n_obs}, sigma={cfg.sigma})")
...
def plot_epoch_figure(
    ...
    clip_y:      bool = False,
    save_path:   Path = Path("outputs/epochs.png"),
    show_plot:   bool = True
) -> None:
    """
    ...
    for idx, epoch in enumerate(epochs):
        ...
        ax.scatter(data["t_obs"], data["y_obs"],
                   ...
                   label=f"Train obs  (N={len(data['t_obs'])})")
    ...
    legend_elements = [
        ...
        Line2D([0], [0], color=BLUE,   lw=0, marker="o", markersize=5,
               label=f"Train observations  "
                     f"(N={len(data['t_obs'])}, sigma={cfg.sigma})"),
