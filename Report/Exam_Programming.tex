...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images_DHO/demo_forward_model_overdamped/summary.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images_DHO/demo_forward_model_underdamped/summary.png}
...
Since the underdamped regime is the most interesting, I will show the rest of the generated plots for this case as well as a demonstration of all plotting methods. The plotting methods are shown on Figures \ref{fig:demo_forward_model_underdamped/analytic_solution} to \ref{fig:demo_forward_model_underdamped/losses}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Images_DHO/demo_forward_model_underdamped/analytic_solution.png}
    \caption{Predicted solution for the underdamped case.}
    \label{fig:demo_forward_model_underdamped/analytic_solution}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Images_DHO/demo_forward_model_underdamped/predicted_solution.png}
    \caption{Predicted solution for the underdamped case.}
    \label{fig:demo_forward_model_underdamped/predicted_solution}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_DHO/demo_forward_model_underdamped/epoch_snapshots.png}
    \caption{Epoch snapshots for the underdamped case.}
    \label{fig:demo_forward_model_underdamped/epoch_snapshots}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images_DHO/demo_forward_model_underdamped/losses.png}
    \caption{Loss convergence plot for the underdamped case.}
    \label{fig:demo_forward_model_underdamped/losses}
\end{figure}

These plots give us some useful insights. The epoch plot (Figure \ref{fig:demo_forward_model_underdamped/epoch_snapshots}) shows how the model learns the equation behavior, starting at low $t$ and working its way through the entire domain. The loss plot (Figure \ref{fig:demo_forward_model_underdamped/losses}) is also very interesting. We can see that the $\mathcal{L}_{i.c.}$ is minimised the fastest, as it is the largest contribution to the total loss function. At that point, the largest contribution to the total is form the data, and the $\mathcal{L}_{data}$ and $\mathcal{L}_{total}$ curves match each other quite well.  In this case $\mathcal{L}_{val}$ stays lower than $\mathcal{L}_{data}$, since the observation points have data applied to them but the validation points do not. Over time, the other loss terms also converge, and at some point the validation loss $\mathcal{L}_{val}$ stops decreasing; this is the point where the model starts overfitting. Since no more significant optimisations are made, the training is stopped.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images_DHO/demo_forward_model_underdamped_extrapblind/summary.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images_DHO/demo_forward_model_underdamped_ML/summary.png}
...
I have chosen to focus on the underdamped case, as it has the most interesting dynamics of the three cases. The bayesian optimisation will howveer be performed on all three cases. As a reminder, Figure \ref{fig:demo_forward_model_underdamped/predicted_solution} shows the predicted solution using the hyperparameters shown in Table \ref{tab:standard_parameters_DHO}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Images_DHO/demo_forward_model_underdamped/predicted_solution.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Images_DHO/demo_forward_model_hyperparameters/low_complexity/predicted_solution.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Images_DHO/demo_forward_model_hyperparameters/high_lr/predicted_solution.png}
    \caption{Predicted solution with $lr=0.05$.}
    \label{fig:demo_forward_model_hyperparameters/high_lr/predicted_solution}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Images_DHO/demo_forward_model_hyperparameters/low_lr/predicted_solution.png}
    \caption{Predicted solution with $lr=0.05$.}
    \label{fig:demo_forward_model_hyperparameters/low_lr/predicted_solution}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Images_DHO/demo_forward_model_hyperparameters/large_phys/predicted_solution.png}
    \caption{Predicted solution with $\lambda_{phys}=100$.}
    \label{fig:demo_forward_model_hyperparameters/large_phys/predicted_solution}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Images_DHO/demo_forward_model_hyperparameters/large_phys_no_extrap/predicted_solution.png}
    \caption{Predicted solution with $\lambda_{phys}=100$ and no collocation points in the extrapolated domain.}
    \label{fig:demo_forward_model_hyperparameters/large_phys_no_extrap/predicted_solution}
\end{figure}

The model does not seem to dit to the oscillatory behavior, but it does decrease exponentially, which means it at least has some notions of what the solution would look like. This was not the case in the previous blind test, where the solution evened out at a constant value (see Figure \ref{fig:demo_forward_model_underdamped_extrapblind/summary}).\\

Finally, I want to see if the model can fit to the solution \ref{without} any observation points. This results in the following prediction:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Images_DHO/demo_forward_model_hyperparameters/large_phys_no_data/predicted_solution.png}
    \caption{Predicted solution with $\lambda_{phys}=100$ and no noisy observation points for training.}
    \label{fig:demo_forward_model_hyperparameters/large_phys_no_data/predicted_solution}
\end{figure}
The model is in fact able to reconstruct the analytic solution without any data points being provided. This does however negat the point of using a neural network, since it is also possible to reconstruct the solution with finite-difference methods (ex. Euler method or Runge-Kutta). PINNs are more applicable to backwards problems, where we need to extract the (unknown) physical parameters from observational data. This means that we need to balance the loss terms so that both the observation points and the physics of the system are taken into account in the training loop. 
...
\end{itemize}
We have chosen to separate the three cases, since the dynamics are very different for over- and underdamped cases which means the optimal hyperparameters will likely also be different. We have chosen to tune the following parameters in the following intervals:
...
\texttt{lr}               & Initial learning rate for  $\hat\zeta$ and $\hat\omega_0$       & $1.5\times10^{-2}$ \\
...
\texttt{$\omega_0$}          & $2.523 rad/s$ & $2.592 rad/s$   \\
\bottomrule
\end{tabular}
\end{table}

Figures \ref{fig:demo_backward_model_overdamped/predicted_solution} and \ref{fig:demo_backward_model_overdamped/parameter_convergence} show the predicted solution and parameter convergence to the true values.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images_DHO/demo_backward_model_overdamped/predicted_solution.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images_DHO/demo_backward_model_overdamped/parameter_convergence.png}
...
\texttt{$\omega_0$}          & $4.000 rad/s$ & $4.350 rad/s$   \\
\bottomrule
\end{tabular}
\end{table}

Figures \ref{fig:demo_backward_model_criticallydamped/predicted_solution} and \ref{fig:demo_backward_model_criticallydamped/parameter_convergence} show the predicted solution and parameter convergence to the true values.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images_DHO/demo_backward_model_criticallydamped/predicted_solution.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images_DHO/demo_backward_model_criticallydamped/parameter_convergence.png}
...
\texttt{$\omega_0$}          & $1.950 rad/s$ & $1.961 rad/s$   \\
\bottomrule
\end{tabular}
\end{table}

Figures \ref{fig:demo_backward_model_underdamped/predicted_solution} and \ref{fig:demo_backward_model_underdamped/parameter_convergence} show the predicted solution and parameter convergence to the true values.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images_DHO/demo_backward_model_underdamped/predicted_solution.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images_DHO/demo_backward_model_underdamped/parameter_convergence.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images_DHO/parameter_estimation_4/scatter_omega_0_full.png}
    ...
    \label{fig:parameter_estimation_2/scatter_full}
\end{figure}
...
\begin{tabular}{lcc}
\toprule
\textbf{Regime} & \textbf{RMSE $\zeta$} & \textbf{RMSE $\omega_0$} \\
\midrule
...
The $RSME$ indeed increases for the critically and overdamped cases. In order to also take the effect of $\omega_0$ into account, we can plot the \textit{parameter space}, where the axes represent $\zeta$ and $\omega_0$. Figure \ref{fig:parameter_estimation_2/scatter_omega_0_full} shows the $(\zeta,\ \omega_0)$ in the parameter space, color coded by the $RMSE$ of the predicted values.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{Images_DHO/parameter_estimation_4/parameter_space_rmse_full.png}
...
This is a powerful method to predict when a shock will occur; a shockwave takes place when two characteristics cross, which is equal to $t = -\frac{1}{\min_x u_x(x,\ t=0)}$. However, we will be concerned with the viscid equation. To get ground truth solutions, we must look at other solving algorithms.
...
\begin{equation}
    ...
    \end{split}
\end{equation}
\subsubsection{Plotting methods}
I ahve implemented a wide range of plotting methods, which are described below:
\begin{itemize}
    ...
    \item A plot of the ground truth and predicted solution at three different times ($t=0,\ t=t_{shock}/2,\ t=t_{shock}$). This is the plot that is explictly requested in the assignment.
    ...
    \begin{enumerate}
        ...
        \item A 2D plot of the data MSE $|\hat u(x, t)-u(x, t)|^2$
        \item A 3D surface plot of the predicted solution with noisy observation points\footnote{As before, these points are not actually used during training but are generated afterwards to give an idea of what the observation points look like.}
        \item A plot of the different loss terms as a funtion of the epochs.
    \end{enumerate}
\end{itemize}
An example of every plot will be shown in section \ref{ch:gauss}.
\subsection{Code demonstration}
In this section, we will demonstrate PINNs for three different initial conditions (Gauss, N-wave and Step-up functions), and three different values for the viscosity ($\nu=0.1,\ 0.05,\ 0.01$). In all three examples, we will use the following hyperparameters:
...
\texttt{n\_obs}               & Observation points                & 30                   \\
\texttt{n\_bc}                & Boundary condition points         & 50                   \\
\texttt{sigma}                & Observation noise $\sigma$        & $1\times10^{-2}$     \\
\bottomrule
\end{tabular}
\end{table}
Just like before, these hyperparameters have been hand-picked and are observed to give reasonable results. We will tune the hyperparameters in section \ref{ch:hyperparameters_BE}.

\subsubsection{Gauss}\label{ch:gauss}
...
\end{figure}
We will now solve the equation for three different values for $\nu$.\\

\paragraph{$\nu=0.05$}
The ground truth solution calculated by the Cole-Hopf transformation is shown on Figure \ref{fig:Gauss_nu_0.05_gt}.
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
...
The predicted solution at the three requested valeus for $t$ (initial condition, before shock and after shock) are shown on Figure \ref{fig:Gauss_nu_0.05/3_times}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images_BE/Gauss_nu_0.05/3_times.png}
...
\end{figure}
We can see that the model is able to fit to the ground truth solution very accurately. For this case, I will also demonstrate the other plotting methods. Figures \ref{fig:Gauss_nu_0.05/gt_2D} and \ref{fig:Gauss_nu_0.05/pred_2D} show the analytic and predicted solutions plotted as 2D matrices.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{Images_BE/Gauss_nu_0.05/gt_2D.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{Images_BE/Gauss_nu_0.05/pred_2D.png}
    \caption{Predicted solutions in 2D for gaussian initial conditions and $\nu=0.05$.}
    \label{fig:Gauss_nu_0.05/pred_2D}
\end{figure}

We again see a very good agreement between the prediction and true solution. Figures \ref{fig:Gauss_nu_0.05/pred_solution} and \ref{fig:Gauss_nu_0.05/pred_grid} show the predicted solution at more values for $t$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{Images_BE/Gauss_nu_0.05/pred_solution.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{Images_BE/Gauss_nu_0.05/pred_grid.png}
    \caption{Predicted solutions at 8 different times for gaussian initial conditions and $\nu=0.05$.}
    \label{fig:Gauss_nu_0.05/pred_grid}
\end{figure}

Figure \ref{fig:Gauss_nu_0.05/losses} shows the losses as a function of the epochs.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{Images_BE/Gauss_nu_0.05/losses.png}
    \caption{Convergece of loss function terms for gaussian initial conditions and $\nu=0.05$.}
    \label{fig:Gauss_nu_0.05/losses}
\end{figure}

Figure \ref{fig:Gauss_nu_0.05/losses} is a summary plot with the most important information about the prediction.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/Gauss_nu_0.05/summary.png}
    \caption{Summary plot fot the predicted solution by a PINN for gaussian initial conditions and $\nu=0.05$.}
    \label{fig:Gauss_nu_0.05/summary}
\end{figure}

For brevity, I will not show all plots for each and every case. For the rest of the demonstration cases, I will only show the predicted solution at the three requested times and the summary plot.

\paragraph{$\nu=0.1$}
Let us now see what happens if the viscosity is increased.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images_BE/Gauss_nu_0.1/3_times.png}
    \caption{Predicted solutions at start time, before shock time and at shock time for gaussian initial conditions and $\nu=0.1$.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/Gauss_nu_0.1/summary.png}
    \caption{Summary plot fot the predicted solution by a PINN for gaussian initial conditions and $\nu=0.1$.}
    \label{fig:Gauss_nu_0.1/summary}
\end{figure}

\paragraph{$\nu=0.01$}
Let us now see what happens if the viscosity is decreased.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images_BE/Gauss_nu_0.01/3_times.png}
    \caption{Predicted solutions at start time, before shock time and at shock time for gaussian initial conditions and $\nu=0.01$.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/Gauss_nu_0.01/summary.png}
    \caption{Summary plot fot the predicted solution by a PINN for gaussian initial conditions and $\nu=0.01$.}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
Where $w$ is the width of the wave and $A$ is the height of the wave. In this case, we have chosen $L=15, \ w=3, \ A=\frac{1}{3}$. Just like with the gaussian intiial conditions, we can predict the time when a shockwave will form by approximating the solution by the inviscid case. This yelds $t_{shock}=3.000$. The method of characteristics is shown on Figure \ref{fig:N_wave_nu_0.1/method_of_characteristics}.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{Images_BE/N_wave_nu_0.1/method_of_characteristics.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images_BE/N_wave_nu_0.05/3_times.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/N_wave_nu_0.05/summary.png}
    \caption{Summary plot fot the predicted solution by a PINN for N-wave initial conditions and $\nu=0.05$.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images_BE/N_wave_nu_0.1/3_times.png}
    \caption{Predicted solutions at start time, before shock time and at shock time for N-wave initial conditions and $\nu=0.1$.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/N_wave_nu_0.1/summary.png}
    \caption{Summary plot fot the predicted solution by a PINN for N-wave initial conditions and $\nu=0.1$.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images_BE/N_wave_nu_0.01/3_times.png}
    \caption{Predicted solutions at start time, before shock time and at shock time for N-wave initial conditions and $\nu=0.01$.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/N_wave_nu_0.01/summary.png}
    \caption{Summary plot fot the predicted solution by a PINN for N-wave initial conditions and $\nu=0.01$.}
...
\end{figure}
Now we will solve the equation for three different values for $\nu$, in the same way as for the gaussian and N-wave initial conditions.
\paragraph{$\nu=0.05$}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images_BE/Step_up_nu_0.05/pred_solution.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images_BE/Step_up_nu_0.1/pred_solution.png}
    \caption{Predicted solutions at four different times for N-wave initial conditions and $\nu=0.1$.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Images_BE/Step_up_nu_0.01/pred_solution.png}
    \caption{Predicted solutions at four different times for N-wave initial conditions and $\nu=0.01$.}
...
\subsubsection{Inclusion of data}
\begin{figure}[H]
    \centering
...
\subsubsection{Extrapolating capabilities}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images_BE/Gauss_nu_0.05_extrapblind/summary.png}
    \caption{Summary plot fot the predicted solution by a PINN for gaussian initial conditions and $\nu=0.05$ when the mdoel is only trained on a fraction of the domain.}
...
\begin{figure}[H]
    ...
    \caption{Summary plot fot the predicted solution by a PINN for gaussian initial conditions and $\nu=0.05$ when the mdoel is only trained on a fraction of the domain, but noisy data points are included in the training.}
...
\begin{figure}[H]
    ...
    \caption{Summary plot fot the predicted solution by a non-physics informed neural network for gaussian initial conditions and $\nu=0.05$.}
...
\begin{tabular}{lccc}
\toprule
\textbf{Parameter} & \textbf{Gauss} & \textbf{N-wave} & \textbf{Step-up} \\
\midrule
\texttt{best\_val}           & $3.86\times10^{-5}$ & $3.67\times10^{-4}$ & $1.19\times10^{-5}$ \\
\midrule
\texttt{hidden}              & 64      & 64      & 48      \\
\texttt{n\_layers}           & 4       & 6       & 6       \\
\texttt{lr}                  & $2.64\times10^{-3}$ & $5.88\times10^{-3}$ & $8.11\times10^{-3}$ \\
\texttt{scheduler\_step}     & 1000    & 4000    & 2000    \\
\texttt{scheduler\_gamma}    & 0.581   & 0.595   & 0.609   \\
\texttt{adam\_beta1}         & 0.965   & 0.852   & 0.915   \\
\texttt{adam\_beta2}         & 0.991   & 0.991   & 0.999   \\
\texttt{lambda\_phys}        & $3.26\times10^{-2}$ & $7.71\times10^{-1}$ & $1.44\times10^{0}$ \\
\texttt{lambda\_ic}          & 19.32   & 24.60   & 1.21    \\
\bottomrule
\end{tabular}
\end{table}
