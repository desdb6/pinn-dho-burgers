...
from plot import plot_epoch_figure, plot_predicted_parameter_convergence
...
def run_single(run_idx: int, device) -> dict | None:
    ...
    plot_predicted_parameter_convergence(
        history=history,
        cfg=cfg,
        output_path=f"{OUTPUT_PATH}/convergence_zeta_{zeta:.4f}_omega_0_{omega_0:.4f}.png",
        show=False
    )
    ...
    plot_epoch_figure(
        ...
        snapshots=snapshots,
        t_plot_full=t_plot,
        output_path=f"{OUTPUT_PATH}/epoch_snapshots_zeta_{zeta:.4f}_omega_0_{omega_0:.4f}.png",
        show=False
    )
