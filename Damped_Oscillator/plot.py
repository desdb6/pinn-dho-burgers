...
def plot_summary(
    ...
    ax_data.set_title(
        "Panel 1 -- Training data: final randomised noisy observations vs true trajectory\n"
        r"$t=0$ excluded from random observations; $y(0)$ and $y'(0)$ enforced via $\mathcal{L}_{ic}$",
    ...
    ax_data.scatter([0], [cfg.y0], color=PURPLE, s=120, marker="*", zorder=7,
                    label=rf"Initial condition  $y(0)={cfg.y0}$  [enforced via $\mathcal{{L}}_{{ic}}$]")
    ax_data.scatter(
...
def plot_epoch_figure(
    ...
    for idx, epoch in enumerate(epochs):
        ...
        # title — consistent style with plot_summary
