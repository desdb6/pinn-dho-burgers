...
def train(model:            nn.Module,
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        l_phys = loss_physics(model, t_col_t) if use_physics else torch.zeros(1, device=DEVICE)
        l_ic   = loss_ic(model, t_ic_t)       if use_ic      else torch.zeros(1, device=DEVICE)
    ...
    if save_path is not None:
        torch.save(model.state_dict(), os.path.join(BASE_DIR, save_path))
